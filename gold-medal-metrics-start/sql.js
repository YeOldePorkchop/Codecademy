...
const orderedMedals = (country, field, sortAscending) => {
  ...
  if (field) {
    orderingString = sortAscending ? `ORDER BY ${field} ASC` : `ORDER BY ${field} DESC`;
  }
...
const orderedSports = (country, field, sortAscending) => {
  ...
  return `
    SELECT sport, COUNT(*) as count, (COUNT(sport) * 100 / (select COUNT(*) FROM GoldMedal WHERE country = '${country}')) AS percent
    FROM GoldMedal
    WHERE country = '${country}'
    GROUP BY sport
    ${orderingString};
